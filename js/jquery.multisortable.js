		...
		return this.each(function() {
			...
			options.start = function(event, ui) {
				if (ui.item.hasClass(settings.selectedClass)) {
					...
					parent.find('.' + settings.selectedClass).each(function(i,item) {
						if($(this).hasClass('pick')){
							prePick_height += height;
						}
						height += Number($(this).outerHeight());							
					});
					ui.placeholder.height(height);
				}
				$('.'+settings.selectedClass).css('margin-top',(prePick_height)+"px");
			...
			options.stop = function(event, ui) {
				$('.pick').removeClass('pick');
				$('.'+settings.selectedClass).css('margin-top',"");
